...
// outer loop to loop through Y coordinate
outer: for (let y = 0; y < arr.length; y++) {
    let re = /\d+/g;
    let results = arr[y].matchAll(re);
    let top;
    let bottom;
    let left;
    let right;

    inner: for (let res of results) {
        let xStr = res.index;
        let xEnd = xStr + res[0].length;
        let xLeft = xStr - 1 > 0 ? xStr - 1 : 0;
        let xRight = xEnd + 1 < arr[y].length ? xEnd + 1 : arr[y].length - 1;
        left = arr[y].slice(xLeft, xStr);
        right = arr[y].slice(xEnd, xRight);

        if (y > 0) {
            top = arr[y - 1].slice(xLeft, xRight);
        }

        if (y < arr.length - 1) {
            bottom = arr[y + 1].slice(xLeft, xRight);
        }
        // check the contents of each slice (top, bottom, left, right)

        function checkPart(part) {
            let partRe = /[^.\d]/;
            console.log(`part is: ${part} and res is: ${res[0]}`);
            if (partRe.test(part)) {
                // console.log(parseInt(res[0]));
                counter += parseInt(res[0]);
                return true;
            }
            return false;
        }

        if (top && checkPart(top)) continue inner;
        if (bottom && checkPart(bottom)) continue inner;
        if (left && checkPart(left)) continue inner;
        if (right && checkPart(right)) continue inner;
    }
}
